...
// easy: 3x4(12칸) 그리드, 6쌍 필요 — public/card/card-{그룹}-{순번}.jpg, 그룹 1~6 각 2장(변형).
...
// hard: 4x4(16칸) 그리드, 8쌍 필요 — 그룹 구분 없이 전체 12장(변형 포함) 중 랜덤 8장을 뽑음
const ALL_CARD_IMAGES = Object.values(CARD_GROUPS).flat()
...
function pickCardImages(difficulty) {
  if (difficulty === 'hard') {
    return shuffle(ALL_CARD_IMAGES).slice(0, 8)
  }
...
function buildDeck(difficulty) {
  return shuffle(
    pickCardImages(difficulty).flatMap((image, i) => [
      { key: `${i}-a`, image },
      { key: `${i}-b`, image },
    ])
  )
...
export function CardFlipGame({ difficulty = 'easy', onClear }) {
  const [deck] = useState(() => buildDeck(difficulty))
  const [flipped, setFlipped] = useState([])
  const [matched, setMatched] = useState([])
  const [locked, setLocked] = useState(false)

  // 카드를 뒤집는 순간 로딩이 시작되면 늦으므로, 이번 판에 실제로 뽑힌 카드만 미리 브라우저 캐시에 받아둠
  useEffect(() => {
  ...
  return (
    <div className={[styles.grid, difficulty === 'hard' ? styles.hard : ''].join(' ')}>
      {deck.map((card, index) => {
